...
public class C38 {
	public static void main(String[] args) {
		...
		Employee empByCity1 = employees.stream().filter(emp->emp.city()=="Bengaluru").findAny().orElseThrow(() -> new RuntimeException ("Employee not found"));
		System.out.println(empByCity1.name());
		Employee empByCity2 = employees.stream().filter(emp->emp.city()=="Hyderabad").findAny().orElseThrow(() -> new RuntimeException ("Employee not found"));
		System.out.println(empByCity2.name());
	}
}
